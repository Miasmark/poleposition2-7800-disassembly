...
local lastst=-1
local E8N,E8BAD,E8STR=0,0,0
TAPS[85]=mem:install_read_tap(0xF15D,0xF15D,"e8",function(a,d) if f>600 and (mem:read_u8(0xB9)&1)==1 and mem:read_u8(0x9D)~=0x13 then
  local fd=mem:read_u8(0xFD)+256*mem:read_u8(0xFE); local e6,e7=mem:read_u8(0xE6),mem:read_u8(0xE7)
  for b=0,12 do local r=6*b+3; local y=mem:read_u8(0xC07E+r); local v=mem:read_u8(fd+y)
    if r>=0x3C then v=(v+mem:read_u8(0xB6))&0xFF end
    v=v|mem:read_u8(0x1F3C+r)
    if e6<0x80 and (e7==0xFF or r>e7) and r<=e6 then v=v|0xE0; E8STR=E8STR+1 end
    local got=mem:read_u8(0x1B4E+r); E8N=E8N+1
    if got~=v then E8BAD=E8BAD+1; if E8BAD<6 then o:write(string.format("E8BAD f%d row=%d got=%02X want=%02X e6=%02X e7=%02X",f,r,got,v,e6,e7)..string.char(10)) end end end end return d end)
emu.register_frame_done(function() if f==tonumber(os.getenv("FR"))-1 then o:write(string.format("E8CHECK rows=%d bad=%d striped=%d",E8N,E8BAD,E8STR)..string.char(10)) end end)
emu.register_frame_done(function()
  f=f+1
  local stq=mem:read_u8(0x9D)
  if f%60==0 or stq~=lastst then lastst=stq
    o:write(string.format("V f%d st=%02X p1score=%s p2score=%s p1lap=%02X%02X.%02X p2lap=%02X%02X run=%d clock=%02X gap=%d A6=%d qst=%d qpos=%d park=%d x1=%d x2=%d | C3=%d A7=%d p2clk=%02X%02X p2lapn=%d rst=%d race=%d",f,stq,hx(0x1CA5,3),hx(0x2770,3),mem:read_u8(0xBE),mem:read_u8(0xBD),mem:read_u8(0xBC),mem:read_u8(0x2775),mem:read_u8(0x2774),mem:read_u8(0x2776),mem:read_u8(0xDF),s16(0x275C),mem:read_u8(0xA6),mem:read_u8(0x2027),mem:read_u8(0x2028),mem:read_u8(0x202B),(function(v) if v>127 then return v-256 end return v end)(mem:read_u8(0xD1)),-(function(v) if v>127 then return v-256 end return v end)(mem:read_u8(0x2702)),mem:read_u8(0xC3),mem:read_u8(0xA7),mem:read_u8(0x2730),mem:read_u8(0x2731),mem:read_u8(0x2733),mem:read_u8(0x2734),mem:read_u8(0x2735))..string.char(10)) end
