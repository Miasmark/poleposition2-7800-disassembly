...
local lastst=-1
local ring={} local rn=0
TAPS[10]=mem:install_write_tap(0x00E6,0x00E7,"e",function(a,v) if f>9300 then rn=rn+1; ring[rn]=string.format("E f%d %s=%02X pc=%04X e6=%02X e7=%02X", f, a==0xE6 and "E6" or "E7", v, cpu.state["PC"].value, mem:read_u8(0xE6), mem:read_u8(0xE7)) end return v end)
TAPS[11]=mem:install_write_tap(0x00A7,0x00A7,"x",function(a,v) if v==0xF0 and not DUMPED then DUMPED=1; for i=math.max(1,rn-30),rn do o:write(ring[i]..string.char(10)) end
  local t={} for s=0,15 do t[#t+1]=string.format("%X:%02X/%02X%02X", s, mem:read_u8(0x19B4+s), mem:read_u8(0x19D4+s), mem:read_u8(0x19C4+s)) end
  o:write("OBJ "..table.concat(t," ").." list="..string.format("AE=%02X AA=%02X ",mem:read_u8(0xAE),mem:read_u8(0xAA))) for i=0,15 do o:write(string.format("%02X ",mem:read_u8(0x19A4+i))) end o:write(string.char(10)) end return v end)
