...
local W1,W2=255,255
TAPS[50]=mem:install_read_tap(0xD719,0xD719,"wk",function(a,v) if cpu.state["PC"].value==0xD719 then
  local c1,c2=mem:read_u8(0xD3),mem:read_u8(0x27C0)
  local g=s16(0x275C)
  if c1~=255 then W1=c1 end
  if c2~=255 then W2=c2 end
  local function one(tag,sl,off,t) if sl==255 then return "" end
    local z=mem:read_u8(0x19C4+sl)+256*mem:read_u8(0x19D4+sl); if z>=32768 then z=z-65536 end
    return string.format(" %s slot=%d type=%02X z=%d spd=%d lat=%d t=%d",tag,sl,mem:read_u8(0x19B4+sl),z+off,mem:read_u8(0x1A10+sl),mem:read_u8(0x1A00+sl),t) end
  local l1=one("P1",W1,0,mem:read_u8(0xD4)); local l2=one("P2",W2,g,mem:read_u8(0x27B5))
  if l1~="" or l2~="" then o:write("W f"..f..l1..l2..string.char(10)) end
  if c1==255 and mem:read_u8(0xD4)==0 then W1=255 end
  if c2==255 and mem:read_u8(0x27B5)==0 then W2=255 end
end return v end)
