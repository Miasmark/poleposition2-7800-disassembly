...
-- can pass too, so check the low byte is a lean ($00-$20). Env: O, END, and
-- PW for the header's palette/width byte ($98; $78 with PP2_HIRES_CAR).
local M=(type(manager.machine)=="function") and manager:machine() or manager.machine
local mem=M.devices[":maincpu"].spaces["program"]
local f,last,shots,hits=0,-10000,0,0
local PW=tonumber(os.getenv("PW") or "0x98")
local o=io.open(os.getenv("O"),"w")
emu.register_frame_done(function() f=f+1
  local found=nil
  for a=0x2200,0x25FC do
    if mem:read_u8(a+1)==PW then local h=mem:read_u8(a+2) if h>=0x70 and h<=0x7F and mem:read_u8(a+3)~=0xA1 then found=a break end end end
