...
local lastst=-1
local CA,CB,SL=tonumber(os.getenv("CA") or "0"),tonumber(os.getenv("CB") or "0"),tonumber(os.getenv("SL") or "3")
TAPS[20]=mem:install_write_tap(0x19C4+SL,0x19C4+SL,"zl",function(a,v) if f>=CA and f<=CB then o:write(string.format("ZL f%d pc=%04X %d->%d D5=%d x=%02X y=%02X stk=%s",f,cpu.state["PC"].value,mem:read_u8(a),v,mem:read_u8(0xD5)+256*mem:read_u8(0xD6),cpu.state["X"].value,cpu.state["Y"].value,(function() local sp=cpu.state["SP"].value local t={} for i=1,10 do t[#t+1]=string.format("%02X",mem:read_u8(0x100+((sp+i)&0xFF))) end return table.concat(t," ") end)())..string.char(10)) end return v end)
TAPS[21]=mem:install_write_tap(0x19D4+SL,0x19D4+SL,"zh",function(a,v) if f>=CA and f<=CB then o:write(string.format("ZH f%d pc=%04X %d->%d",f,cpu.state["PC"].value,mem:read_u8(a),v)..string.char(10)) end return v end)
