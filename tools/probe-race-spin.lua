...
local lastst=-1
local SPA,SPB=tonumber(os.getenv("PA")),tonumber(os.getenv("PB"))
local SC={}; local t0=0; local FP=1/59.92
TAPS[42]=mem:install_read_tap(0xDB8E,0xDB8E,"spin",function(o,d)
  if f>=SPA and f<SPB then
    local sp=cpu.state["SP"].value
    local r=mem:read_u8(0x100+((sp+1)&0xFF))+256*mem:read_u8(0x100+((sp+2)&0xFF))
    local line=math.floor((M.time:as_double()-t0)/FP*263/8)*8
    local k=string.format("ret=%04X line=%03d",r,line); SC[k]=(SC[k] or 0)+1 end
  return d end)
emu.register_frame_done(function() t0=M.time:as_double()
  if f==SPB then local po=io.open(os.getenv("SPIN_OUT"),"w"); for k,c in pairs(SC) do po:write(k.." "..c..string.char(10)) end po:close() end end)
