...
local lastst=-1
local seen={}; local pend={}
emu.register_frame_done(function() local st=mem:read_u8(0x9D)
  if not seen[st] then seen[st]=f; for _,d in ipairs({2,20,60}) do pend[f+d]=st end end
  if pend[f] then M.video:snapshot(); o:write(string.format("SNAPST f%d st=%02X",f,pend[f])..string.char(10)) end end)
