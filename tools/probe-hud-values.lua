-- probe-hud-values.lua -- no recording; both players scripted as in
-- probe-collide-scenario.lua (PHASES, NAVN, P2C, P2P; SNAPS / AUTOSNAP
-- screenshots), plus "V" lines every 60 frames and at every state change:
-- both scores (player 1's $1CA5-$1CA7, player 2's P2_SCORE), both lap times,
-- player 2's lap-running flag, the clock and the gap. The driver's notes follow.
...
emu.register_frame_done(function()
  ...
  if f%60==0 or stq~=lastst then lastst=stq
    o:write(string.format("V f%d st=%02X p1score=%s p2score=%s p1lap=%02X%02X.%02X p2lap=%02X%02X run=%d clock=%02X gap=%d",f,stq,hx(0x1CA5,3),hx(0x2770,3),mem:read_u8(0xBE),mem:read_u8(0xBD),mem:read_u8(0xBC),mem:read_u8(0x2775),mem:read_u8(0x2774),mem:read_u8(0x2776),mem:read_u8(0xDF),s16(0x275C))..string.char(10)) end
